...
public:
  ...
  void gen_sql() override
  {
    ...
    if(datatype != "TINYINT" &&
      ...
      datatype != "BIGINT")
    {
      throw std::runtime_error(std::format("Datatype '{}' is not supported by MariaDB.", datatype));
    }
...
public:
  ...
  void gen_sql () override
  {
    ...
    sql_segment += datatype;
    if (length == 0 && (datatype != "TINYTEXT" && datatype != "TEXT" && datatype != "MEDIUMTEXT" && datatype != "LONGTEXT"))
      throw std::runtime_error(std::format("Length attribute is required for datatype '{}'", datatype));
    if (datatype != "TINYTEXT" && datatype != "TEXT" && datatype != "MEDIUMTEXT" && datatype != "LONGTEXT")
...
public:
  ...
  void gen_sql() override
  {
    ...
    if (enable_default)
    {
      sql_segment += (default_value ? " DEFAULT TRUE": " DEFAULT FALSE");
    }
  }
...
public:
  std::size_t max_n {};
  BinaryField() = default;
  BinaryField(std::string datatype, std::size_t size=0, bool not_null = false, bool unique=false, bool pk=false)
  :FieldAttr("int", datatype, not_null, unique, pk), max_n(size)
  {
    gen_sql();
  }
  ...
  void gen_sql() override
  {
    ...
    {
      throw std::runtime_error(std::format("Datatype '{}' doesn't exist in MariaDB::BinaryField", datatype));
    }
    sql_segment = datatype;
    if (datatype != "BINARY")
    {
      if (max_n != 0) {
        sql_segment += "(" + std::to_string(max_n) + ")";
      }else {
        throw std::runtime_error(std::format("Size must be provided for datatype '{}'!", datatype));
      }
    }
    if(not_null) sql_segment += " NOT NULL";
  }
  ...
  void track (std::string new_model_name, const std::string col_name, FieldAttr& old_col_obj,
              std::ofstream& Migrations, const nlohmann::json& frm = {}) const override
  {
    BinaryField& bin_obj = dynamic_cast<BinaryField&>(old_col_obj);
    if (bin_obj.datatype != datatype)
    {
      if (datatype == "VARBINARY") {
        db::alter_column_type(new_model_name, col_name, "VARBINARY("+std::to_string(max_n)+")", Migrations);
      } else {
        db::alter_column_type(new_model_name, col_name, "BINARY", Migrations);
      }
    }
    if (bin_obj.max_n != max_n && datatype == "VARBINARY") {
        db::alter_column_type(new_model_name, col_name, "VARBINARY("+std::to_string(max_n)+")", Migrations);
    }
  }
...
public:
  ...
  void gen_sql() override
  {
    ...
    if(datatype != "DATE" && datatype != "TIME" && datatype != "DATETIME" &&
      datatype != "TIMESTAMP" && datatype != "YEAR")
    {
      throw std::runtime_error(std::format("Datatype '{}' not supported in MariaDB::DateTimeField.", datatype));
    }
...
public:
  ...
  ForeignKey() = default;
	ForeignKey(std::string cn, std::string mn, std::string rcn, std::string ctype="int",
            std::string rsql="INTEGER NOT NULL", std::string on_del="RESTRICT", std::string on_upd="RESTRICT")
              ...
              std::ofstream& Migrations, const nlohmann::json& frm) const override
  {
    try
    {
      ...
      Migrations<<"ALTER TABLE " + new_model_name + " ";
